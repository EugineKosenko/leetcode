...
fn min_time_to_reach(move_time: Vec<Vec<i32>>) -> i32 {
    ...
    let mut queue = BinaryHeap::from([(Reverse(0), 1, (0, 0))]);
    while let Some((Reverse(time), delta, point)) = queue.pop() {
        ...
        for (dr, dc) in [(-1, 0), (0, 1), (1, 0), (0, -1)] {
            ...
            let time = time.max(move_time[next.0][next.1]) + delta;
            match visited.get(&next) {
                None => { visited.insert(next, time); queue.push((Reverse(time), 3-delta, next)); },
                Some(&t) => { if time < t {
                    visited.insert(next, time); queue.push((Reverse(time), 3-delta, next));
                }}
            }
        }
    }
